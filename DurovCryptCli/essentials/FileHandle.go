...
func FilePathInput() (string, error) {

	//ask user for fileName for encrypt
	filePath, err := InputPrompt("Enter a FilePath/FileName to encrypt: ")
	if err != nil {
		return "", err
	}
...
func FileWrite(cipherText []byte, fileName string) string {
	if len(cipherText) < 0 {
		return fmt.Sprintf("Error cipherText is empty.")
	}

	basepath := filepath.Base(fileName)
	extension := filepath.Ext(fileName)
	filename := basepath[:len(basepath)-len(extension)]

	err := os.WriteFile(filename, cipherText, 0644)
	if err != nil {
		return fmt.Sprintf("Can't write Data to file! %v\n", err)
	}
	return
}
